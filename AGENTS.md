...
- `Api::ExpensesController#index` orders by `created_at DESC` and filters the month by `created_at`, although the domain field for "when the expense happened" is `date` — this mismatch is the subject of BUG-001 in `TICKETS.md`.
